...
let state = 'ready', bird, pipes, score, frame, pipeTimer, lastTime = 0, muted = false;
...
function update(time) { const elapsed=lastTime?time-lastTime:16.67;lastTime=time;const speedFactor=Math.min(2.5,Math.max(.5,elapsed/16.67));if(state==='playing'){frame+=speedFactor;pipeTimer+=speedFactor;const difficulty=Math.min(score,12);const gravity=.25+difficulty*.006;const pipeSpeed=2.25+difficulty*.08;bird.vy+=gravity*speedFactor;bird.y+=bird.vy*speedFactor;bird.tilt=Math.min(Math.PI/2.8,bird.vy*.075);if(pipeTimer>=Math.max(84,104-difficulty*2)){const gap=110+Math.random()*255;pipes.push({x:W+10,gap,scored:false});pipeTimer=0;}pipes.forEach(p=>{p.x-=pipeSpeed*speedFactor;if(!p.scored&&p.x+58<bird.x){p.scored=true;score++;chirp(740,.07);}const touchesPipe=bird.x+bird.r>=p.x&&bird.x-bird.r<=p.x+58&&(bird.y-bird.r<=p.gap||bird.y+bird.r>=p.gap+154);if(touchesPipe)endGame();});pipes=pipes.filter(p=>p.x>-70);if(bird.y+bird.r>=H-35||bird.y-bird.r<=0)endGame();}else if(state==='ready'){bird.y=H*.45+Math.sin(Date.now()/340)*8;bird.tilt=-.12;} draw();requestAnimationFrame(update); }
